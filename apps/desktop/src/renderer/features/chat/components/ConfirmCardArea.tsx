...
export function ConfirmCardItem({
  ...
  let planPhase: 'wait_confirm' | 'running' | 'completed' | 'cancelled' | 'wait_dangerous' | 'modified' = 'wait_confirm';
  if (resolvedEntry) {
    if (entry.state === 'cancelled') planPhase = 'cancelled';
      ...
      planPhase = statuses.length > 0 && statuses.every((s) => s.status === 'success') ? 'completed' : 'running';
    }
  }
        ...
        {isActionCard(entry as never) ? (
          <ActionCard
            ...
            onResolve={(choiceId, rememberMode) =>
              resolve(id, choiceId, choiceId === 'confirm' ? '确认' : choiceId === 'modify' ? '修改计划' : '取消', rememberMode !== null, rememberMode ?? 'session')
            }
          />
          ...
          <PlanCard
            entry={{
              ...
              steps: ((entry.request.steps as { id?: string; name?: string; title?: string; tools?: string[] }[]) ?? []).map((step, index) => ({
            ...
            onResolve={(choiceId, choiceLabel) =>
              resolve(
                id,
                choiceId,
                choiceLabel ?? (choiceId === 'confirm' ? '按当前方案执行' : choiceId === 'modify' ? '调整方案' : '取消任务'),
                false,
                'session',
              )
            }
          ...
          <ConfirmCard
            ...
            onResolve={(choiceId: string, rememberMode?: 'session' | 'always' | null) => {
              const choices = (entry.request.choices as { id: string; label?: string }[] | undefined) ?? [];
...
export function ConfirmCardArea({ matchedTurnIds }: { matchedTurnIds?: Set<string> }) {
  ...
  const allEntries = useMemo(() => {
    ...
    if (matchedTurnIds && matchedTurnIds.size > 0) {
      merged = merged.filter((entry) => !(entry.request.turn_id && matchedTurnIds.has(entry.request.turn_id)));
    }
