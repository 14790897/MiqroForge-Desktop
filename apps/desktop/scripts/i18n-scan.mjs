...
function contextKind(src, start) {
  ...
  let best = null;
  let m;
  while ((m = CONTEXT_RX.exec(win)) !== null)
    best = { kind: m[1] ?? m[2] ?? m[3] ?? m[4] ?? m[5], idx: m.index };
  if (!best) return null;
...
function blankLiterals(src) {
  ...
  for (const lit of scanLiterals(src)) {
    for (let i = lit.start; i <= lit.end && i < chars.length; i++)
      if (chars[i] !== '\n') chars[i] = ' ';
  }
...
// ---------------------------------------------------------------------------
const isTest = (p) =>
  /(^|\/)(test|tests|spec|__tests__|e2e)(\/|$)/.test(p) ||
...
for (const file of files) {
  ...
  for (const lit of scanLiterals(src)) {
    ...
    if (/^[0-9A-Za-z_\-./\\()\[\]{}%$#@!? ]+$/.test(st) && !CJK.test(st.replace(/[%{}]/g, '')))
      continue;
    ...
    const am =
      /(?:^|[^\w:-])(placeholder|title|aria-label|aria-description|alt|description|helperText|label)\s*=\s*["'`]$/.exec(
        prev
      );
  ...
  for (const h of scanJsxText(src, blanked)) {
    rows.push({
      file,
      line: lineAt(src, h.start),
      kind: 'jsx-text',
      module: module_,
      risk: h.text.includes('{') ? 'interp' : '',
...
const INTERNAL = rows.filter(
  (r) =>
    r.kind === 'log-internal' || (r.kind === 'multiline-blob' && !CJK.test(r.text.slice(0, 120)))
);
const uiRows = rows.filter((r) =>
  !r.kind.startsWith('attr') ? UI_KINDS.has(r.kind) && r.kind !== 'multiline-blob' : true
);
const all = rows;
...
console.log(
  `  risks: ${Object.entries(byRisk)
    .map(([k, v]) => k + '=' + v)
    .join('  ')}`
);
console.log(
  `reports written: ${rel(join(process.cwd(), OUT_DIR))}/summary.md, unique-strings.md, full.tsv`
);
