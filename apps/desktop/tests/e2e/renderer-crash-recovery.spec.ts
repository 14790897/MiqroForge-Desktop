...
async function readRendererState(
  electronApp: ElectronApplication
): Promise<{ noticeCount: number; noticeText: string | null; inputReady: boolean } | null> {
  const deadline = Date.now() + RELOAD_TIMEOUT_MS;
  ...
  while (Date.now() < deadline) {
    ...
    lastError = '渲染层尚未就绪或 webContents 仍处于崩溃态';
    await new Promise((r) => setTimeout(r, POLL_INTERVAL_MS));
  }
  console.log(`[diagnostic] readRendererState 超时：${lastError}`);
  return null;
}
...
test.describe('Issue #1035 — 渲染进程崩溃后自动重载与恢复提示', () => {
  ...
  test('崩溃后自动重载并插入恢复提示（期望行为 1~4）', async () => {
    ...
    const state = await readRendererState(electronApp);
    expect(state, '重载后应能读到渲染层 DOM').not.toBeNull();
  ...
  test('预算内再次崩溃：自动重载继续，attempt 递增', async () => {
    ...
    const state = await readRendererState(electronApp);
    expect(state, '第二次重载后应能读到渲染层 DOM').not.toBeNull();
