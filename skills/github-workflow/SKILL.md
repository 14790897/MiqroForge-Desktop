...
| 流水线 | 触发 | 行为 |
|--------|------|------|
| `.github/workflows/weekly-release.yml` | 每周五 09:00 (Asia/Shanghai) / 手动 `workflow_dispatch` | 自动创建 develop→main 发布 PR（标题 `chore(release): merge develop into main`）并**立即合并**，随后 release.yml 的 semantic-release 自动发版打包 |
